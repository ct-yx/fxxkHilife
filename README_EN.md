...
- **v2.8.2 fixes**: sound quality preference now syncs after the first toggle; ANC awareness mode shows the correct sub-mode title/options; the permission screen now includes notification permission plus background/autostart setting shortcuts.
...
| HUAWEI FreeBuds 7i | Temporary conservative profile, full adaptation pending | v2.8.2 adds a reduced capability table to lower initialization pressure; full 7i support will be continued with testers in the next major compatibility round for more models and vendors |
...
3. App version (v2.8.2+ logs include it automatically)
