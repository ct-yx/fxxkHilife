...
当前已完成的是 **BT-0.1 的本地协议/能力代码审计与资料存在性核对**；这不代表所有型号和所有指令都已经实机验证。连接速度实测尚未完成，因此 BT-0.2 仍保持未开始。
...
| BT-0.2 连接速度实机基线 | [~] 进行中 | 已加入 attemptId/阶段计时/触发来源日志；仍需同一设备每个场景至少 10 次，产出 P50/P95、失败原因和阶段耗时 |
