...
>
> 当前开发测试版本：v4.7.1 / versionCode 105，2026-08-26（有效壁纸 + Android 13 使用共享光场、自适应透明度和自研 AGSL 柔光玻璃；无壁纸、壁纸未完成加载和 Android 12 及以下使用优化后的标准 Material 3）
>
...
当前 **BT-0 至 BT-4 已完成**：`v4.3.10 / 98` 是 BT 重构后的首个公开大版本，主验证设备的蓝牙状态契约门已通过，但结论仍限定在已有型号/固件/系统证据范围内，不外推为所有型号和所有指令均已实机验证。旧第三方渲染路线已撤销；`v4.6.0 / 103` 保留为标准 Material 3 和深色模式壁纸亮度正式基线，`v4.7.0 / 104` 为第一版集中式 AGSL 玻璃历史测试包，`v4.7.1 / 105` 在其基础上加入共享光场、自适应透明度、无边框受光和悬浮顶部栏。玻璃只在有效壁纸且 Android 13+ 时启用，低版本和无效壁纸保持 MD3 fallback；UI 仍需通过设备截图/交互和无障碍定向门后再标记完成，不重复蓝牙全量矩阵。
...
| UI-1 设计令牌与渲染基础 | [~] 目标受阻：等待 Light Field AGSL 玻璃定向实测 | 2026-08-26；v4.7.1 统一卡片 profile、共享低分辨率壁纸纹理、方向性光场、自适应透明度、无边框边缘受光和 MD3 fallback；等待 API 33+ 截图、API 32 fallback 与滚动性能证据 |
...
| UI-4 设备功能页面 | [~] 目标受阻：等待定向卡片/展开交互实测 | 2026-08-26；v4.7.1 保持同一 shader 节点，统一设备卡片、可展开选项卡、箭头旋转、触摸光场和内容对比度自适应；只需验证本页滑动、展开 10 次、状态更新和资源显示，不复跑 BT 矩阵 |
| UI-5 设置、持久化与兼容层收口 | [~] 目标受阻：等待实测报告 | 2026-08-15；SettingsRepository、更新检查/下载/安装状态和 UpdateCard 已接入，仍需持久化、无障碍和更新流程验证 |

### 0.4 应用版本号方案

当前正式发布版本为 **versionName `4.6.0` / versionCode `103`**；当前开发测试版本为 **`4.7.1 / 105`**。`4.6.0 / 103` 是标准 Material 3 与深色模式壁纸亮度正式基线；`4.4.0 / 99` 至 `4.4.2 / 101` 的第三方玻璃尝试只作为历史版本保留，`4.7.0 / 104` 是第一版集中式 AGSL 玻璃测试门，`4.7.1 / 105` 是 Light Field V2 的定向测试门。更早的 BT 版本仍按下表保留，BT-0 的审计、研究、日志和测试准备不单独发版。
...
| UI-2 typed State / Event / Navigation | 4.7.2 | 106 | `UI_STATE_EVENT`：页面状态、动作反馈、导航事件和兼容投影 |
...
- GitHub Actions 的手动构建必须填写或保留阶段标签；当前默认 `UI_LIGHT_FIELD_GLASS_V2`，产物名、测试报告名和 metadata 都带该标签，避免只看版本号无法区分 UI 测试包。
...
`v4.7.1 / 105` 的 Light Field 玻璃测试门不改变业务页面 API。`Material3Card` 默认请求 `WallpaperGlass`，由 `SurfaceRenderer` 根据 `LocalWallpaperGlass`、壁纸加载状态和 API 级别决定实际 renderer：
...
- shader 只作用于材料背景，不扭曲文字、图标或按钮；边缘通过方向性受光、弱暗部、宽泛光和轻微采样位移表达，不再依赖矩形描边。
- 页面级 `LightFieldState` 读取根坐标的触摸位置，只更新 shader uniform；TopBar 使用更明显的主题黑/白偏置，并只在玻璃采样层加入约 `2–3dp` 的低频扭曲，标题和操作控件位置保持稳定。
- 背景局部复杂度和文字对比风险参与 `adaptiveAlpha`/`adaptiveTint`，高对比区域自动增加材质遮罩，降低内容被壁纸吞没的概率。
...
本轮代码完成后进入 `UI_LIGHT_FIELD_GLASS_V2` 测试门；测试返回前只保持上述受阻状态，不标记 UI 完成，也不重复 BT A-F/36/100 项矩阵。
## 6. 验收标准
...
- UI 测试报告使用 `UI_LIGHT_FIELD_GLASS_V2`、`UI_API32_MD3_FALLBACK`、`UI_LIGHT_FIELD_TOUCH`、`UI_NAVIGATION_SESSION`、`UI_STATE_EVENT_TARGETED`、`UI_SETTINGS_PERSISTENCE`、`UI_ACCESSIBILITY_MATRIX` 和 `UI_RELEASE_AUDIT`；不以版本号单独作为阶段识别。
