...
### 背景驱动液态玻璃重构
- 修正 v2.10.x 液态玻璃“白色图层感”过强的问题：玻璃卡片不再依赖整块白色内层来制造效果。
- `LiquidGlassCard` 的 Haze tint alpha 被压低，只保留极弱染色，让背景本身通过 `hazeSource` / `hazeEffect` 成为主要视觉来源。
- `Surface` 主体透明度大幅降低，避免背景上覆盖半透明白板。
- 移除卡片内容区整块白色渐变背景，保留真实背景模糊与内容之间的直接关系。
- `liquidGlassOptics` 不再绘制整块高光/暗边填充，改为只绘制边缘描边、左上局部高光线、局部折射光带和 Fresnel 边缘线。
- `LiquidGlassPanel` 同步降低白色背景与 tint，避免 ANC 面板遮蔽壁纸。
- 目标视觉从“白色磨砂卡片”调整为“背景驱动的玻璃折射卡片”。
...
### 最小修复版
- 修复设置页与手势设置页顶部栏在液态玻璃模式下不透明的问题，补充 transparent `TopAppBar` colors。
- 优化设备页初始化状态：未同步真实 `ancMode` 前不显示 ANC 模块，避免默认 `normal` / options 被误展示为真实状态。
- ANC 子模式与音质偏好必须同时具备当前值和 options 才显示，避免连接初期出现空选项或假状态。
- 删除设置页中的「传统」渲染方案用户选项；传统渲染代码仍保留为内部兜底，配置加载固定使用 Haze 2 最新主线。
- 本版基于 v3.0.0 最小改动，不包含此前回退的根背景重构。
...
### 后台同步状态提示
- `DeviceProps` 新增 `pendingInitHandlers`，把仍在后台 retry 的初始化项暴露给 UI。
- 连接后若仍有后台补齐项，设备页在电量卡片下显示「后台同步中」提示卡。
- 提示卡会用中文列出正在补齐的内容，例如设备信息、双击手势、滑动手势、摘下暂停、语音语言等。
- 提示文案说明通常 15–45 秒内陆续完成，慢项会保持后台重试，避免用户误以为功能消失。
- 连接完成后立即同步一次 pending 状态，确保提示能及时出现；当后台 retry 全部成功后提示自动消失。
...
### 连接初始化优化
- 根据 FreeBuds 6i 实测日志优化核心状态 fast init：battery / ANC / low latency / sound quality / in-ear 改为小间隔并发发起，避免单个 1.5s 超时串行拖慢首屏。
- 新增 TX mutex，仅串行化 SPP 写入本身，不阻塞不同 responseId 的并发等待，降低并发初始化时输出流交错风险。
- 自动低延迟写入改为等待 low_latency 状态首次读取完成后再尝试，避免连接初期写入抢占核心状态读取通道。

### 详情页进入门槛
- 连接建立后不再立即进入设备详情页。
- 必须至少获取 ANC 状态、低延迟状态和电池状态后，才自动进入详情页。
- 从保存设备或扫描设备点击连接时，也会先停留在主页/连接状态，避免未初始化完成时进入二级界面。

### 电池卡 UI 重做
- 设备页电池卡重做为左侧双耳机、右侧电池盒布局。
- 左耳/右耳使用放大的几何耳机造型，一上一下错位展示，并在各自底部显示电量。
- 右侧单独展示电池盒造型和盒电量/充电状态。
...
### 电池与设备状态视觉
- 主页已保存设备中，系统蓝牙未连接的耳机图标增加斜杠，并使用弱化颜色区分未连接状态。
- ANC 三段控件中放大透传图标，修正有效内容偏小的问题。
- 设备页电池卡左/右耳机圆形区域增加按电量高度填充的水位效果。
- 耳机盒图标中间窄条改为按电量从左到右填充的进度效果。

### 导航与初始化补齐
- 修复进入设备详情页后返回主页会被自动再次跳回详情页的问题；用户手动返回后，本次连接不会再强制自动进入。
- 连接流程增加 `connectingJob` 防重入，避免同一设备短时间内重复 RFCOMM connect。
- 后台非核心 handler 补齐间隔从固定 30s 优化为前几轮 8s、后续 20s，并限制每轮非核心重试数量，减少长串超时导致的等待。
- 前台 fast poll 获取 device_info 成功后会从 failedHandlers 移除，避免重复进入后台补齐队列。
...
### 连接状态与进入详情速度修复
- 主页顶部连接横幅在核心状态未齐前不再显示“已连接”，改为“正在同步核心状态…”，避免误导用户。
- 修复 SPP 收包中出现粘包/组合包时只处理外层包的问题：现在会扫描并分发内嵌 `5a00` 子包。
- 该修复可让日志中已经返回但被包在组合帧里的 battery / ANC / low_latency 回包及时消费 pendingResponses，减少等待重试导致的详情页进入延迟。
...
### 电池卡单耳未连接标识修复
- 设备详情页电池卡中，左/右任意一只耳机只要未上报或电量为 0%，都会在对应耳机图标上叠加斜杠标识。
- 解决只显示 0% 但图标缺少未连接视觉提示的问题。
...
### 发布
- 新增听音统计页面：按耳机连接时长累计总时长、今日时长、已听天数、连续天数，并提供近 16 周活动热力图入口
- versionCode: 67
- versionName: 3.8.0
- tag: v3.8.0
...
### 日志与快捷开关
- 分享日志和日志保留设置不再依赖耳机控制通道，断开状态也可直接导出历史日志。
- Quick Settings Tile 在控制通道已连接但 ANC 尚未初始化时显示中性未知态，不再把未知状态误报为“关闭”。
- 未初始化时点按 Tile 会直接设为降噪；仓库同步乐观状态后，这次手动操作即作为 ANC 初始化结果。

### 连接发现与初始化提速
- 根据 2000 行历史日志分离两次连接会话：旧会话在 21:43 断开；新会话 03:33 建立 RFCOMM 仅耗时 116ms，主要延迟来自系统蓝牙连接发现和核心状态串行等待。
- Android 13+ 蓝牙系统广播改用 exported 动态接收，并接入 A2DP / HEADSET Profile 已连接设备快照；广播和 Profile 事件直接携带设备建立 SPP，不再被周期退避拦截。
- 周期兜底检查改为静默低频执行，避免大量“未连接”日志挤掉真正的连接时间线。
- 恢复 v2.1.3 早期实测有效的 80ms 错峰并发思路，但仅用于 responseId 各不相同的核心 handler；可选 handler 仍在后台顺序补齐，避免全量并发导致 6i 丢包。
- RFCOMM 连接后增加 250ms 设备就绪窗口；核心首轮每项仅尝试一次，失败项再以并发方式补一次，单个电量超时不再阻塞 ANC 和低延迟初始化。
- 同 responseId 请求继续互斥，但槽位等待纳入调用总超时，并按 deferred 实例安全释放，避免等待无边界或旧请求误删新请求。
...
## 紧急提醒， 目前软件连接部分存在恶性bug
## anc按钮正常 如需正常使用，请降级到3.7x
...
### FreeBuds 6i 初始化与详情页
- 将电池、ANC、低延迟和音质的核心 SPP 查询改为严格串行；接收侧和所有发包路径共用请求锁，避免后一个命令覆盖前一个命令的回包。
- 核心状态首轮与一次重试的最坏预算为 9.75 秒；可选能力探测采用短时 best-effort，不再因为失败而持续显示“后台同步中”。
- 控制通道建立后立即允许进入耳机详情页；页面会持续渲染已获得的部分状态，不再因单个核心字段暂缺被退回首页。

### 自动低延迟与诊断
- 自动低延迟只依赖 low_latency 状态，不再被电池、ANC 等无关核心查询失败阻断。
- 低延迟写入改为短时写入加读回确认，自动重试窗口收敛到 10 秒；日志记录请求排队耗时、写入确认和读回不匹配原因。
...
### FreeBuds 6i 优先控制路径
- 根据 v4.2.5 实机日志确认：SPP 请求已无排队，但首轮 `2b2a` / `2b6c` 读取仍可能没有回包；此前的自动低延迟因此在 `lowLatency == null` 时被跳过。
- 恢复“连接即优先执行”语义：控制通道完成 250ms 就绪窗口后，自动低延迟先发送写入与短时读回，不再等待初始低延迟读取成功。
- ANC、低延迟、音质调整到电池查询之前，避免电池超时占用用户最先需要的控制状态。
- 首次优先写入后，低延迟在核心读取完成后继续做 10 秒内的有限确认重试；新增写入 ACK、读回和失败原因日志。
...
### 本轮实现
- 新增 `GlassScrollableContent` 共享滚动视口：每个 LazyColumn 最多创建一个稳定的真实 typed `hazeGlass` 背景，列表行固定走 draw-only 玻璃皮肤，不逐行创建 captured-content effect。
- Home、Device、Settings、Scan、Gesture 和 ListeningStats 的列表从首次组合起固定 renderer；滚动时不再切换 effect/source，列表行也不再读取 `isScrolling`，减少触摸边界的批量重组。
- `GlassHost` 的背景 source 全程保持挂载；新增共享视口 effect 就绪条件测试，继续保留 `surfaceRole`、`sourceAttached` 和 `effectSurfaceCount` 诊断。
- 本轮只处理 UI-ERR-001/002，不修改 BT-0 至 BT-4，也不复跑蓝牙 A-F/36/100 项矩阵。
- 静态检查和 GitHub Actions 仍是下一道门；未执行本地 Gradle，等待 CI 包后的 Home/Device/Settings 定向截图与滚动性能报告。
