...
> **当前版本：v2.8.3**
> 修复 ANC 切换后旧状态包导致的模式乱跳；保留音质偏好同步、ANC 子模式显示与权限引导修复。
...
- **v2.8.3 修复**：音质偏好首次切换后可及时同步；ANC 切换到通透后下方标题与选项显示为“通透模式”；权限引导页加入通知权限、后台保活与自启动设置入口。
...
| HUAWEI FreeBuds 7i | 临时保守适配，待完整适配 | v2.8.3 起保留保守能力表以降低初始化压力，并暂时隐藏未确认可用的自动暂停选项；完整适配会在下一轮大版本“更多型号/更多厂商耳机适配”中与测试者继续推进 |
