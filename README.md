...
> **当前版本：v4.1.0**
> 继续削薄 SppDriver，新增 HuaweiPendingResponseManager 与 HuaweiHandlerInitializer，将 pending response 与 Handler 初始化调度从驱动中抽出。
...
- **低延迟 / 游戏模式**：支持手动开关；也可在设置中启用自动低延迟，耳机重新连上手机并完成初始化后立即尝试开启，失败时每 500ms 重试，最多 30s，适合游戏/视频场景。
...
| HUAWEI FreeBuds 7i | 临时保守适配，待完整适配 | v4.1.0 起保留保守能力表以降低初始化压力，并暂时隐藏未确认可用的自动暂停选项；完整适配会在下一轮大版本“更多型号/更多厂商耳机适配”中与测试者继续推进 |
