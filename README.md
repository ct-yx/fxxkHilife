...
## v4.7.0：自研 AGSL 壁纸柔光玻璃测试包

- 卡片、可展开卡片和顶部栏通过共享低分辨率壁纸纹理获得实时区域取色；API 33+ 使用自研 AGSL 绘制宽边缘泛光、折射和顶部轻微扭曲。
- Android 12 及以下、无壁纸或壁纸加载失败时回退为优化后的标准 Material 3，不使用软件折射。
- 测试构建标签为 `UI_AGSL_GLASS_V1`；本包只进入 UI 定向截图、滚动、无障碍和展开交互验收，不重复 BT 矩阵。
