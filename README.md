...
> **当前版本：v2.7.4**
> 修复音质偏好首次切换状态不同步、ANC 降噪/通透子模式选项混乱；权限引导页加入通知权限与后台/自启动设置入口。
...
- **v2.7.4 修复**：音质偏好首次切换后可及时同步；ANC 切换到通透后下方标题与选项显示为“通透模式”；权限引导页加入通知权限、后台保活与自启动设置入口。
